...
function mount-nfs() {
    ...
    if [ -z $(echo `dpkg -la 2>/dev/null`|grep nfs-common) ]; then
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :m:h OPTION; do
            case "${OPTION}" in
                ...
                h) echo_label 'description'; echo_primary 'Mount nfs shared ressource into local mount point'
                    echo_label 'usage';      echo_primary 'mount-nfs [server]:[resource] -m [mount_point] -h (help)'
                    return 0;;
