...
function unset-hosts() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :i:h OPTION; do
            case "${OPTION}" in
                i) IP_ADDRESS="${OPTARG}";;
                h) echo_label 'description'; echo_primary 'Comment out desired host from /etc/hosts'
                    echo_label 'usage';      echo_primary 'unset-hosts [hosts] -i [ip] -h (help)'
                    return 0;;
    ...
    # escape "." from ip
    ESC_IP="$(echo ${IP_ADDRESS}|sed 's/\./\\\./g')"
