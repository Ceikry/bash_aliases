    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :kh OPTION; do
            case "${OPTION}" in
                ...
                h) echo_label 'description'; echo_primary 'Creates symfony var/cache, var/logs, var/sessions'
                    echo_label 'usage';      echo_primary 'rebuild (project_directory) -k (create .gitkeep) -h (help)'
                    return 0;;
