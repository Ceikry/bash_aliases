    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :u:g:dkopvh OPTION; do
            case "${OPTION}" in
                ...
                v) VENDORS='true';;
                h) echo_label 'description'; echo_primary 'Removes symfony cache, logs, sessions and vendors'
                    echo_label 'usage';      echo_primary 'nuke (project_directory) -u [user] -g [group] -d (recreate directories) -k (create .gitkeep) -o (own) -p (set permissions) -v (delete vendors) -h (help)'
                    return 0;;
