    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :a:c:h OPTION; do
            case "${OPTION}" in
                ...
                c) CHANNEL="${OPTARG}";;
                n) WRITE='false';;
                h) echo_label 'description'; echo_primary 'Dump captured packets from target adapter'
                    echo_label 'usage';      echo_primary 'wifi-sniff [target] [-c channel] (-a adapter) -n (no_write) -h (help)'
