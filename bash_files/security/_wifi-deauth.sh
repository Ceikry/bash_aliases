...
function wifi-deauth() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :c:p:r:h OPTION; do
            case "${OPTION}" in
                ...
                r) ROUTER="${OPTARG}";;
                h) echo_label 'description'; echo_primary 'Disconnect client from wifi network'
                    echo_label 'usage';      echo_primary 'wifi-deauth (adapter) -r [router_mac] -c [client_mac] -p [packet_count] -h (help)'
    ...
    if [[ ! "${CLIENT}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        ...
        echo_label 'usage'; echo_primary 'wifi-deauth (adapter) -r [router_mac] -c [client_mac] -p [packet_count] -h (help)'
        echo_label 'note'; echo_primary "available adapters: ${ADAPTERS}"
        return 1
    fi
