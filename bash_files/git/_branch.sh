...
function branch() {
    ...
    while [ "${#}" -gt 0 ]; do
        OPTIND=0
        while getopts :dDrh OPTION
        do
            case "${OPTION}" in
                d) DELETE='true';;
                D) DELETE_REMOTE='true';;
                r) RENAME='true';;
                h) echo_label 'description'; echo_primary 'Create, checkout, rename or delete branch'
                    echo_label 'usage';      echo_primary 'branch [name] [-d delete] [-D delete_remote] -r (rename) -h (help)'
                    return 0;;
