...
function backup() {
        ...
        while getopts :bglHSs:u:h OPTION; do
            case "${OPTION}" in
                ...
                h) echo_label 'description:'
                    echo_primary 'Backup current local repository to remote server'
                    echo_label 'usage:'
                    echo_info 'backup [repository] [-u username] [-s server] -b (bitbucket) -g (github) -l (gitlab) -H (https) -S (ssh) -h (help)'
                    return 0;;
    ...
    if [ "${GIT_SSH}" = 'true' ]; then
        ...
        echo_info "git remote add origin https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git"
        git remote add origin https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git
    fi
