...
function gdiff() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                c) COMMIT="${OPTARG}";;
                h) echo_label 'description'; echo_primary 'Print changes between working tree and latest commit'
                    echo_label 'usage';      echo_primary 'gdiff [file_name] [-c commit_hash] -h (help)'
                    return 0;;
