...
function push() {
    ...
    while [ "${#}" -gt 0 ]; do
        OPTIND=0
        while getopts :afrh OPTION; do
            case "${OPTION}" in
                a) COMMIT='true';;
                f) FORCE='true';;
                r) RECURSIVE='true';;
                h) echo_label 'description'; echo_primary 'Update remote git repository'
                    echo_label 'usage';      echo_primary 'push (message) -a (commit_all) -f (force) -r (recursive) -h (help)'
                    return 0;;
    ...
    # message is the only accepted argument
