...
function reclone() {
    ...
    do
        case "${OPTION}" in
            H) GIT_SSH='false';;
            S) GIT_SSH='true';;
            h) echo_label 'description'; echo_primary 'Deletes current git repository and clones it again from remote'
                echo_label 'usage'; echo_primary 'reclone -H (https) -S (ssh) -h (help)'
                return 0;;
    ...
    if [ "${GIT_SSH}" = 'true' ]; then
        ...
        echo_info "git clone https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git ${TOPLEVEL}"
        git clone https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git ${TOPLEVEL}
    fi
