    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                a) ADD='true';;
                r) RENAME='true';;
                h) echo_label 'description'; echo_primary 'Write changes to repository, rename last commit'
                    echo_label 'usage';      echo_primary 'commit (message) -a (add all) -r (rename commit) -h (help)'
                    return 0;;
