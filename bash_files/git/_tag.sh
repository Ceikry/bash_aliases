...
function tag() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                ...
                l) LIST='true';;
                p) PUSH='true';;
                h) echo_label 'description'; echo_primary 'Create, list or return latest tag'
                    echo_label 'usage';      echo_primary 'tag [tag] [-m message] -c (commit) -f (fetch) -l (list) -p (push) -d (delete) -D (delete remote) -h (help)'
                    return 0;;
    ...
    if [ -z "${TAG}" ]; then
        if [ "${DELETE}" = 'true' ] || [ "${DELETE_REMOTE}" = 'true' ]; then
            ...
            if [ "${DELETE_REMOTE}" = 'true' ]; then
                echo_info "git tag | xargs git push --delete origin"
                git tag | xargs git push --delete origin
            fi

            # delete all local tags
            if [ "${DELETE}" = 'true' ]; then
                echo_info "git tag | xargs git tag --delete"
