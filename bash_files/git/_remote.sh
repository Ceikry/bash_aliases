...
function remote() {
    ...
    if [ -n "$(git remote get-url origin 2>/dev/null)" ]; then
        ...
        # get ssh config
        if [ -n "$(git remote get-url origin | grep -o 'git@')" ]; then
            GIT_SSH='true'
        fi
    fi
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :s:u:bglHSh OPTION; do
            case "${OPTION}" in
                ...
                h) echo_label 'description'; echo_primary 'Set / print local git repository configuration'
                    echo_label 'usage';      echo_primary 'remote (repository) [-s server] [-u username] -b (bitbucket) -g (github) -l (gitlab) -H (https) -S (ssh) -h (help)'
                    return 0;;
    ...
    if [ "${GIT_SSH}" = 'true' ]; then
        ...
        echo_info "git remote ${URL} origin https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git"
        git remote ${URL} origin https://${GIT_SERVER}/${GIT_USERNAME}/${GIT_REPOSITORY}.git
    fi
