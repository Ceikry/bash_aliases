...
function bitbucket() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                ...
                h) echo_label 'description'; echo_primary 'Create new private git repository on bitbucket.org'
    ...
    echo_prompt "Enter bitbucket password: "
    read -s GIT_PASSWORD
    echo -e "\n"
    ...
    echo `curl --user ${GIT_USERNAME}:${GIT_PASSWORD} https://api.bitbucket.org/2.0/repositories/${GIT_USERNAME}/${GIT_REPOSITORY} --header "Content-Type: application/json" --data '{ "scm": "git", "is_private": true, "fork_policy": "no_forks" }'`
}
