...
function compress() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :hr OPTION; do
            case "${OPTION}" in
                r) RECURSIVE='true';;
                h) echo_label 'description'; echo_primary 'Compress a file, a folder or each subfolders into separate archives recursively '
                    echo_label 'usage';      echo_primary 'compress [source] [destination] -r (recursive) -h (help)'
                    return 0;;
    ...
    local DESTINATION="`realpath "${ARGUMENTS[((${LBOUND} + 1))]}"`"
    ...
    if [ -d "${SOURCE}" ] && [ "${RECURSIVE}" = 'true' ]; then
        # cache file count
        local TOTAL=$(find "${SOURCE}" -maxdepth 1 ! -path "${SOURCE}" -type d | wc -l)
        local COUNT=1

        # ! -path "${SOURCE}" = avoid listing "."
        find "${SOURCE}" -maxdepth 1 ! -path "${SOURCE}" -type d | while read FOLDER_PATH; do
