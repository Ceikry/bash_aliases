    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                ...
                h) echo_label 'description'; echo_primary 'rename files from given folder (creates undo script)'
                    echo_label 'usage';      echo_primary 'rename [folder] -r (recursive) -d (include directories) -a (preserve accents) -e (remove extra char) -s (snake_case) -t (train-case) -l (lower case) -u (UPPER CASE) -h (help)'
                    return 0;;
