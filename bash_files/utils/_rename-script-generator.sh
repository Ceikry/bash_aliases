...
function rename-script-generator() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                d) DIRECTORY='true';;
                h) echo_label 'description'; echo_primary 'generate rename script in current folder'
                    echo_label 'usage';      echo_primary 'rename-script-generator [folder] -d (include directories) -h (help)'
                    return 0;;
    ...
    find "${SOURCE}" -maxdepth 1 ! -name 'rename.sh' ! -name 'README.md' ! -path "${SOURCE}" | while read FILE_PATH; do
    ...
    find "${SOURCE}" -maxdepth 1 ! -name 'rename.sh' ! -name 'README.md' ! -path "${SOURCE}" | sort -t '\0' -n | while read FILE_PATH; do
