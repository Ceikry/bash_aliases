...
function move-all-files-here() {
    ...
    if [ "${#}" -gt 2 ]; then
        ...
        echo_label 'usage'; echo_primary 'move-all-files-here [source] (destination)'
        return 1
    fi
    ...
    find "${SOURCE}" ! -path "${SOURCE}" ! -name 'move-all-files-here_undo.sh' -type f | while read FILE_PATH; do
