...
function mp3-update-tag() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        do
            case "${OPTION}" in
                h) echo_label 'description'; echo_primary 'Update mp3 tag from filename (creates undo script)'
                    echo_label 'usage';      echo_primary 'mp3-update-tag [folder] -h (help)'
                    return 0;;
    ...
    find "${SOURCE}" -type f -name '*.mp3' | while read FILE_PATH; do
