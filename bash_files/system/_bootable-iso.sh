...
function bootable-iso() {
    ...
    while [ "${#}" -gt 0 ]; do
        ...
        while getopts :h OPTION; do
            case "${OPTION}" in
                h) echo_label 'description'; echo_primary 'Create bootable usb drive from iso file or generate iso file from source'
                    echo_label 'usage';      echo_primary 'bootable-iso [source] [device (eg: /dev/sdb)] -h (help)'
                    return 0;;
    ...
    if [ -n "${SOURCE}" ] && [ -n "${DESTINATION}" ]; then
        ...
        echo_prompt 'Are you sure you want to continue ? (yes/no) [no]: '
        read USER_PROMPT
